...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const normalizedAddress = { province: cleanText(address.province, 80), city: cleanText(address.city, 100), postalCode: cleanText(address.postalCode, 12), street: cleanText(address.street, 120), streetNumber: cleanText(address.streetNumber, 30), apartment: cleanText(address.apartment, 80), references: cleanText(address.references, 300), agencyId: cleanText(address.agencyId, 30), agencyName: cleanText(address.agencyName, 120), agencyAddress: cleanText(address.agencyAddress, 200), meetingPoint: cleanText(address.meetingPoint, 120), notes: cleanText(address.notes, 500) };
      if (delivery.method === "correo-sucursal" && (!normalizedAddress.agencyId || !normalizedAddress.agencyName || !normalizedAddress.agencyAddress)) throw Object.assign(new Error("Elegí una sucursal válida de Correo Argentino."), { statusCode: 400 });
      const shipping = { type: cleanText(delivery.method, 40), province: normalizedAddress.province, city: normalizedAddress.city, postalCode: normalizedAddress.postalCode, street: normalizedAddress.street, number: normalizedAddress.streetNumber, floorApartment: normalizedAddress.apartment, reference: normalizedAddress.references, branch: { id: normalizedAddress.agencyId, name: normalizedAddress.agencyName, address: normalizedAddress.agencyAddress }, meetingPoint: cleanText(delivery.meetingPoint || normalizedAddress.meetingPoint, 120) };
