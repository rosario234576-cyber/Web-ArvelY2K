...
const FACEBOOK_PAGE_TOKEN_HANDOFF_PATH = ".facebook-page-access-token";
const FACEBOOK_USER_TOKEN_HANDOFF_PATH = ".facebook-long-lived-user-token";
...
const configuredFacebookUserToken = String(process.env.FACEBOOK_LONG_LIVED_USER_TOKEN || "").trim();
const configuredFacebookPageId = String(process.env.FACEBOOK_PAGE_ID || "").trim();
...
function sanitizeError(error) {
  ...
  for (const secret of [configuredFacebookToken, configuredFacebookUserToken, configuredInstagramToken, process.env.INSTAGRAM_APP_SECRET, process.env.META_APP_SECRET]) {
...
async function refreshFacebookPageToken() {
  if (tokenMode !== "facebook-page" || !configuredFacebookUserToken) return null;
  ...
  if (!appId || !appSecret) {
    console.warn("::warning::No se puede renovar el token de PÃ¡gina: faltan META_APP_ID o META_APP_SECRET.");
    return null;
  }

  try {
    // Los Page access tokens se vuelven a emitir a partir de un token de usuario
    // de larga duraciÃ³n. Nunca se guarda ninguno de esos secretos en el repositorio.
    const exchangeUrl = new URL(`${FACEBOOK_GRAPH_ORIGIN}/${FACEBOOK_GRAPH_VERSION}/oauth/access_token`);
    exchangeUrl.searchParams.set("grant_type", "fb_exchange_token");
    exchangeUrl.searchParams.set("client_id", appId);
    exchangeUrl.searchParams.set("client_secret", appSecret);
    exchangeUrl.searchParams.set("fb_exchange_token", configuredFacebookUserToken);

    let userToken = configuredFacebookUserToken;
    try {
      const exchanged = await requestJson(exchangeUrl);
      if (exchanged.access_token) {
        userToken = String(exchanged.access_token);
        if (userToken !== configuredFacebookUserToken) {
          await writeFile(FACEBOOK_USER_TOKEN_HANDOFF_PATH, userToken, { encoding: "utf8", mode: 0o600 });
        }
      }
    } catch (error) {
      // Meta can reject a re-exchange while the token is still recent. The existing
      // long-lived user token remains usable to retrieve an updated Page token.
      if (error?.reconnectRequired) throw error;
      console.warn(`::warning::No se pudo reemitir el token de usuario de Meta: ${publicMessage(error)}`);
    }

    const pagesUrl = new URL(`${FACEBOOK_GRAPH_ORIGIN}/${FACEBOOK_GRAPH_VERSION}/me/accounts`);
    pagesUrl.searchParams.set("fields", "id,name,access_token,instagram_business_account");
    pagesUrl.searchParams.set("limit", "100");
    pagesUrl.searchParams.set("access_token", userToken);
    const pagesResult = await requestJson(pagesUrl);
    const pages = Array.isArray(pagesResult?.data) ? pagesResult.data : [];
    const matchingPage = pages.find((page) =>
      (configuredFacebookPageId && String(page.id) === configuredFacebookPageId)
      || String(page.instagram_business_account?.id || "") === instagramUserId
    );
    if (!matchingPage?.access_token) {
      throw new SyncError("No se encontrÃ³ la PÃ¡gina de Facebook vinculada a esta cuenta de Instagram.", {
        category: "configuration",
        reconnectRequired: true
      });
    }
    accessToken = String(matchingPage.access_token);
    await writeFile(FACEBOOK_PAGE_TOKEN_HANDOFF_PATH, accessToken, { encoding: "utf8", mode: 0o600 });
    return { refreshed: true, pageId: String(matchingPage.id) };
  } catch (error) {
    // If the Page token configured today still works, do not take the feed down
    // merely because Meta refused an early token exchange. inspectFacebookToken()
    // below remains the authority on whether it is usable.
    accessToken = configuredFacebookToken;
    if (configuredFacebookToken) {
      console.warn(`::warning::No se pudo renovar el token de PÃ¡gina: ${publicMessage(error)}`);
...
async function main() {
  ...
  try {
    ...
    const facebookRefresh = await refreshFacebookPageToken();
    ...
    await atomicJsonWrite(STATUS_PATH, {
      ...
      token: token ? {
        ...
        warning: refresh && !refresh.refreshed ? "No se pudo renovar el token en este intento." : (facebookRefresh && !facebookRefresh.refreshed ? "No se pudo renovar el token de PÃ¡gina en este intento." : "")
      },
      error: null
    });
    ...
    // Los archivos de traspaso estÃ¡n en .gitignore. El workflow los consume y
    // elimina despuÃ©s de guardar los secretos renovados; no deben eliminarse
    // antes de esa etapa.
  }
}
