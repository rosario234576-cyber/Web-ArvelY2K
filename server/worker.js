...
export default {
  async fetch(request, env) {
    ...
    if (!env.ASSETS) {
      return new Response("Arvel Custom API", {
        status: 200,
    ...
    try {
      ...
      if (response.status === 404 && request.method === "GET") {
        ...
        if (indexResponse.status === 200) {
          return secureResponse(new Response(indexResponse.body, {
            status: 200,
            headers: {
      ...
      const fallbackResponse = await env.ASSETS.fetch(new Request(new URL("/index.html", url).toString(), request));
      return secureResponse(new Response(fallbackResponse.body, {
        status: 200,
        headers: {
          "content-type": "text/html; charset=utf-8",
          "cache-control": "no-cache"
