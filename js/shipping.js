(function () {
  ...
  async function getAgencies(stateId) {
    ...
    if (!response.ok) throw new Error(result.error || "No pudimos consultar las sucursales.");
    return Array.isArray(result.agencies) ? result.agencies : [];
  }

  window.ArvelShipping = Object.freeze({ estimate, normalizePostalCode, getAgencies });
})();
